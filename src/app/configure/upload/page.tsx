...
const page = () => {
  ...
  return (
    <div className={cn('relative h-full flex-1 my-16 w-full rounded-xl bg-gray-900/5 p-2 ring-1 ring-inset ring-gray-900/10 lg:rounded-2xl flex justify-center flex-col items-center',
        ...
        <div className="flex relative flex-1 items-center justify-center w-full">
            <Dropzone 
            ...
            >
              {({getRootProps, getInputProps})=>(
                <div className="flex flex-col h-full w-full flex-1 items-center justify-center"{...getRootProps()}>
                    <input {...getInputProps()}/>
                    {isDragOver ? 
                    (<MousePointerSquareDashed className='size-7 text-zinc-500 mb-2' />) : 
                    isUploading || isPending ? (
                    <Loader2 className='animate-spin size-7 text-zinc-500 mb-2' />
                    ) : (
                    <Image className='size-7 text-zinc-500 mb-2' />
                    )}
